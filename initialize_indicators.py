...
def main():
    ...
    if es.indices.exists(index=apikey):
        ...
        for pulse in pulses[0:300]:
            print(
                "Have pulse already?: "
                + str(
                    es.exists(index=apikey, doc_type="pulse", id=int(pulse["id"], 16))
                )
            )
            if es.exists(index=apikey, doc_type="pulse", id=int(pulse["id"], 16)):
                pulse_match = es.get_source(
                    index=apikey, doc_type="pulse", id=int(pulse["id"], 16)
                )
                ...
                print(
                    "Adding location data to indicators for pulse ID: "
                    + str(int(pulse["id"], 16))
                )
                add_loc_to_indicators(pulse)
                es.index(
                    index=apikey, doc_type="pulse", id=int(pulse["id"], 16), body=pulse
                )
    ...
    # If index does not already exist, create the index and begin loading mapping & pulse information
        ...
        es.indices.put_mapping(
            index=("indicators" + str(apikey)), doc_type="pulse", body=mapping
        )
        ...
        es.indices.put_mapping(
            index="indicators" + str(apikey), doc_type="indicator", body=mapping
        )
...
def validate_apikey():
    ...
    try:
        ...
        raise Exception(
            "Failed to create OTXv2 instance with provided API key:", str(apikey)
        )
    try:
        pulses = otx.getall()
    except:
...
def handle_requests():
    ...
    if method == b"stylokey":
        ...
        result = str.encode(
            str(stylo_key.run_keyword_stylometry_on_text(ctrl, [comp1, comp2, comp3]))
        )
        ...
        result = str.encode(
            str(
                stylo_ngrams.run_stylometry_on_text(
                    ctrl, [comp1, comp2, comp3], number_of_ngrams=1
                )
            )
        )
